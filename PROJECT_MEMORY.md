...
- Veröffentlichung `2.1.47` vorbereitet: Standardgültigkeit neuer Online-Schlüssel in Oberfläche und Erstellungslogik auf `Unbegrenzt` geändert. Bestehende Ablaufdaten bleiben unverändert; Backup-API `2.0.5` benötigt keine Änderung. Installer und Portable-Build lokal erzeugt und gegen Quellmodule und Update-Manifest geprüft; Veröffentlichung und abschließende Anbieterprüfung noch ausstehend.
...
- Zuletzt veröffentlichter Funktionsstand: Version `2.1.46`, Release-Commit `b78a8c1`; annotierter Tag auf beiden Plattformen identisch.
...
- Neue Online-Backups unterstützen `24 Stunden`, `3 Tage`, `7 Tage`, `31 Tage` und `Unbegrenzt` (neuer Standard in Oberfläche und Erstellungslogik; nach `2.1.46` geändert, noch nicht veröffentlicht). Bestehende Schlüssel behalten ihre Laufzeit. Endliche Schlüssel werden lokal aus dem verschlüsselten Schlüsselbund entfernt und serverseitig ab Ablauf nicht mehr wiederhergestellt; der Dienst räumt abgelaufene Datensätze bei Zugriff oder der nächsten Speicherung auf.
...
- Keine offenen Veröffentlichungsschritte für Release `v2.1.46`; Rollback-Ziel ist Anwendungsversion `2.1.45`. Das oben dokumentierte DoodStream-Uploadproblem bleibt offen. Privaten Wiederherstellungsschlüssel zusätzlich unabhängig vom lokalen PC sichern.
